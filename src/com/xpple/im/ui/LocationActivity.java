...
public class LocationActivity extends BaseActivity implements
	...
	private void initBaiduMap() {
		...
		if (type.equals("select")) {// 选择发送位置
			initTopBarForBoth("位置", R.drawable.btn_chat_send_selector,
			...
			initTopBarForBoth("巧遇附近", R.drawable.ic_top_bar_category,
					new onRightImageButtonClickListener() {
						@Override
						public void onClick() {
							// TODO Auto-generated method stub
							startAnimActivity(new Intent(LocationActivity.this,
									NearPeopleActivity.class));
						}
					});
			ll_play = (RelativeLayout) findViewById(R.id.ll_play);
			...
			mMarker = new Marker[size];
			username = intent.getStringArrayExtra("username");
			nick = intent.getStringArrayExtra("nick");
			loc_left = intent.getDoubleArrayExtra("loc_left");
			loc_right = intent.getDoubleArrayExtra("loc_right");
			for (int i = 0; i < size; i++) {
				LatLng mPosition = new LatLng(loc_left[i], loc_right[i]);
				String mNick = nick[i];
				initOverlay(mPosition, mNick, i);
			}
			user_nick.setText(nick[0]);
			mBaiduMap.setOnMarkerClickListener(new OnMarkerClickListener() {
				@Override
				public boolean onMarkerClick(final Marker marker) {
					for (int i = 0; i < size; i++) {
						if (marker == mMarker[i]) {
							user_nick.setText(nick[i]);
							ii = i;
						}
					}
					return true;
			...
			play_btn.setOnClickListener(new OnClickListener() {
				...
				public void onClick(View arg0) {
					...
					intent.putExtra("username", username[ii]);
					intent.setClass(LocationActivity.this, PuzzleActivity.class);
	...
	public void initOverlay(LatLng mPosition, String mNick, int i) {
		OverlayOptions ooA = new MarkerOptions().position(mPosition)
				.icon(bdgeo).zIndex(15);
		mMarker[i] = (Marker) (mBaiduMap.addOverlay(ooA));

		// 添加文字
		OverlayOptions ooText = new TextOptions().fontSize(20).text(mNick)
				.position(mPosition);
